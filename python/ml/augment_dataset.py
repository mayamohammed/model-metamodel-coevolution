...
LABEL_COL  = "label"
TARGET_MIN = 80    # raised from 60
NOISE_STD  = 0.3   # lower noise = more precise samples
...
# Discriminative patterns per confused class
LABEL_PATTERNS = {
    "ECLASS_REMOVED": {
    ...
    for label, grp in df.groupby(LABEL_COL):
        ...
        for i in range(needed):
            if label in LABEL_PATTERNS and i < needed // 2:
                # half from discriminative pattern
                row = make_pattern_sample(feat_cols, label, rng)
            else:
                # half from noise injection on existing samples
